  ...
  return (
    ...
    <div className="min-h-screen flex flex-col relative">
      <header className="fixed w-full top-0 z-50 shadow lg:h-20 lg:px-16 px-6 bg-white flex flex-wrap items-center lg:py-0 py-2">
        ...
        <div className={`${isMenuOpen ? '' : 'hidden'} flex z-50 lg:flex lg:flex-row flex-col lg:items-center lg:w-auto w-full h-full`} id="menu">
          <nav>
            <ul className="lg:flex items-center h-full justify-between text-base lg:mb-0">
              {
                menus.map(menu => (
                  <li key={menu.name} className="lg:mb-0"><a href={menu.link} className={`${currentPath === menu.path ? 'border-b-2' : ''} text-black lg:p-4 py-3 px-0 hover:border-b-2  font-mono border-stone-700`}>{menu.name}</a></li>
                ))
              }
            </ul>
          </nav>
